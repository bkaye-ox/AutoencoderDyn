...
class systemSelectorEnum(Enum):
    ...
    def BILINEAR(nonLinearInputChar=False):
        dynamicModel = LinearSystem(
            nonLinearInputChar=Option.nonLinearInputChar)
        U, Y, U_val, Y_val = dynamicModel.prepareDataset(10000, 1000)
        return dynamicModel, U, Y, U_val, Y_val

    def METRA():

        df = pd.read_feather('data/ts.feather')
        y_source = df['o2pp'].to_numpy()
        u_source = np.array(list(zip(df['o2duty'], df['apduty'])))

        split = int(0.95*len(df))

        y, u = y_source[:split], u_source[:split]
        y_val, u_val = y_source[split:], u_source[split:]

        return DummyModel(), u, y, u_val, y_val
...
class Options():
    def __init__(self):
        ...
        self.stringDynamicalSystemSelector = str(self.dynamicalSystemSelector).replace(
            '<function systemSelectorEnum.', '').split(' at ')[0]
...
if len(sys.argv) > 3:
    if int(sys.argv[3]) == 1:
        ...
        Option.dynamicalSystemSelector = systemSelectorEnum.METRA
        Option.closedLoopSim = False



    Option.stringDynamicalSystemSelector = str(Option.dynamicalSystemSelector).replace(
        '<function systemSelectorEnum.', '').split(' at ')[0]
...
model.trainModel()
predictedLeft, stateLeft, oneStepAheadPredictionError, forwardedPredictedError, forwardError = model.model.predict([
                                                                                                                   inputY, inputU])
...
def openLoopValidation(validationOnMultiHarmonic=True, reset=-1, YTrue=None, U_Vn=None):
    ...
    for i in range(0, finalRange):
        ...
        if YTrue is None:
            ...
            x0RealSystem = np.reshape(
                x0RealSystem_, (simulatedSystem.stateSize,))
        else:
            y_kReal = YTrue[i]
            u = [U_Vn[i]]
            pass
    ...
    fit = 1-(a/b)
    NRMSE = 1-np.sqrt(np.mean(np.square(np.array(logY) -
                      np.array(logYR))))/(np.max(logYR)-np.min(logYR))
    fit = np.max([0, fit])
    ...
    if Option.enablePlot:
        plt.figure()
        plt.title('open loop simulation from k=' +
                  str(openLoopStartingPoint)+" fit="+str(fit))
        y, = plt.plot(logY)
        yr, = plt.plot(logYR)
...
for r in reset:
    for voM in validationOnMultiHarmonic:
        ...
        fit, NRMSE, logY, logYR = openLoopValidation(
            validationOnMultiHarmonic=voM, reset=r, YTrue=YtrueToPass, U_Vn=U_Vn.copy())
        end = time.time()
...
#    bounds=[(-1,1) for i in range(0,MPCHorizon)]
    ...
    for i in range(0, 400):
        x0 = model.convEncoder.predict([pastY.T, pastU.T])
        r = [0.7*np.array([[np.sin(j/(20+0.01*j))]]
                          )+.7 for j in range(i, i+MPCHorizon)]
#        if i>200:
#            r=1+r*0
#        else:
#            r=-1+r*0
#        r=np.array([[.5+1.5*np.sin(i/(50+i/100))]])
    ...
    #    r=np.array([[1.5+np.sin(i/(50+i/100))]])
        ...
        for _ in range(0, Option.TRsteps):
            ...
            def lamdaCostFunction(x): return costFunction(
                x, r, u[0][0], logAB, logC, x0)
            result = optimize.minimize(
                lamdaCostFunction, pastRes, bounds=bounds)
...
scipy.io.matlab.savemat('dump{0}{1}.mat'.format(Option.stringDynamicalSystemSelector, Option.nonLinearInputChar), {
